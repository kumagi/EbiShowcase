...
各ページは既定で3回独立にレビューし、過半数と引用根拠が一致した指摘だけを採用します。

対応バックエンド:

- **LM Studio**（既定）— `http://127.0.0.1:1234/v1`
- **Ollama** — LAN上のホスト（例: `192.168.3.58:11434`）の `/v1`

デフォルトは1バッチだけのドライランです。提案を表示するだけで、フォームへは送信しません。
...
```sh
export OLLAMA_HOST=192.168.3.58:11434
...
```sh
python3 scripts/ai_feedback_crawler.py --once --max-pages 1 \
  --lm-base-url http://192.168.3.58:11434/v1 \
  --provider ollama \
  --model qwen3.6:latest
```
...
```sh
python3 scripts/ai_feedback_crawler.py --once --max-pages 1 \
  --ollama-host 192.168.3.58 --model qwen3.6:latest
```
...
3回のレビューは既定で並列実行します。GPUメモリやOllama側の同時実行設定に合わせて直列化するには
`--review-workers 1`、合議を無効にして比較確認するには `--reviews 1 --review-workers 1` を指定します。
`--reviews` は多数決が成立する正の奇数だけを受け付けます。

```sh
python3 scripts/ai_feedback_crawler.py --once --max-pages 1 \
  --ollama-host 192.168.3.58 --model qwen3.6:latest \
  --reviews 3 --review-workers 3
```

各ページの個別票（gate id、verdict、引用、修正案、エラー）は状態DBの `review_votes` に保存されます。
採用結果だけを見て原因が分からない場合も、3票の食い違いを後から調査できます。
...
切り詰め、ページのフォームと同じ隠し項目も送信します。誤送信を防ぐため、送信は1回25ページまで、
`--watch` や `--force` との併用は禁止です。同じページ・内容・レビュー観点の結果は再送信しません。
...
```sh
python3 scripts/ai_feedback_crawler.py --watch --max-pages 24 --interval-seconds 600 \
  --ollama-host 192.168.3.58 --model qwen3.6:latest
```

停止は `Ctrl-C` です。サイトやフォームに負荷をかけないよう、既定でページ間1秒、巡回間隔10分を
設定しています。定期巡回は安全のためドライラン専用です。内容を確認したうえで、必要な範囲だけを
別の1回実行に `--submit` を付けて送信してください。
...
`--lens` を省略した場合は、安価なローカルモデルが判断を混同しないよう、起動時に
LLM 用ゲートからランダムに1件を選び、その実行中の全ページを同じ1観点でレビューします。選ばれたIDは
`[lenses:random]` として標準エラーへ表示されます。前回と異なる組み合わせ
なら、ページ本文が同じでも新しい観点として再レビューします。

各ゲートには `FAIL ONLY WHEN`、`DO NOT FLAG`、`EVIDENCE REQUIRED` があり、共通ポリシーと
ともにモデルへ渡されます。fail / warn の `evidence` が実際のページ本文からの完全一致引用でない場合、
送信前の検証で pass に戻します。見えていない `main.go`、共有パッケージ、実行画面を推測した指摘は送信されません。
さらに、3票中2票以上が fail / warn で、2票以上が同じ引用箇所を指した場合だけ指摘を採用します。
過半数がなければ pass、問題票が過半数でも引用箇所が一致しなければ誤検出候補として pass に倒します。
URLから判定できるページ種別（core / track / vfx / build / graduation / guide）と対象言語が
ゲートの `applies_to` / `languages` に合わない場合は、モデルを呼ばずに pass とします。
...
python3 scripts/ai_feedback_crawler.py --once --max-pages 3 --force \
  --ollama-host 192.168.3.58 --model qwen3.6:latest \
  --lens pedagogy.code-matches-impl
```

`--lens` は family または完全な gate id を受け取ります。family（例: `pedagogy`）を指定した場合も、
安価なモデルを1観点に集中させるため、そのfamilyから1件だけを実行ごとに選びます。
完全なidを指定すれば同じ観点を固定できます。`node scripts/check-quality-gates.mjs --lenses …` と同じ catalog を読み、
