...
function hub(lang) {
  ...
  const c = ja ? {
    ...
    lead: "Draw は現在のゲーム状態（game）を画面へ投影する処理であり、描画結果は状態とアセットから一意に定まります。そのため、原則としてユニットテストの対象にしません。入力検知・時間経過・乱数・衝突判定・得点計算を担う Update を、薄い入出力アダプタと Ebitengine に依存しない純粋ルール関数へ分離します。本コースの12章では、実際の Update 実装からルールを抽出し、AAA（Arrange-Act-Assert）パターンを用いて各ルールを独立した Go テストで検証します。",
    input: "Updateが入力を読む", pure: "純粋関数へ普通の値を渡す", state: "Updateがgameへ結果を書く", noDraw: "Drawはgameを読むだけ",
    why: "なぜDraw()は原則テストしない？", whyBody: "同一のゲーム状態・アセット・解像度に対して常に同一の描画を行う参照透過な Draw を前提とすれば、ゲームルールの正しさは game のフィールド値によって検証できます。ピクセル単位の画像比較はフォント・GPU・OS環境の違いによって壊れやすく、衝突判定や得点計算の成否を直接証明できません。カスタムシェーダーや画像生成処理の検証には別枠のゴールデン画像テストを適用し、Update のユニットテストとは分離します。",
    update: "Update()をどう構造化する？", updateBody: "Update の処理は次の4段階に整理します。①Ebitengine の API から入力を検知する、②入力情報をプリミティブな数値や小さな構造体に変換する、③Ebitengine 非依存の純粋関数に値を渡して次のゲーム状態を計算する、④計算結果を game 構造体のフィールドへ代入する。純粋関数は Ebitengine パッケージを import せず、時刻や乱数も引数として受け取るため、画面ウィンドウを起動せずに高速な単体テストを実行できます。",
