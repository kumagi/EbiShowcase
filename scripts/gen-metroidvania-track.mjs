...
const phaseSummary=(lang,q)=>lang==="ja"?`${q.s.join(" → ")} をこの順で、1tickごとに実行します。`:`Run ${q.s.join(" → ")} once per tick in this order.`;
...
const lessons = [
  { slug: "camera-rooms", ja: {s:['世界座標を更新', 'カメラ差を計算', '10%追従', '画面へ描画'], t: "世界座標と追従カメラ", l: "画面4枚分の世界を歩き、世界座標からカメラ座標を引いて一部分だけ描きます。", c: "世界と画面を別の座標にする", d: "主人公は世界のどこにいるかを持ち、DrawだけがcameraXを引きます。差の10%ずつ追えば滑らかなカメラになります。" }, en: {s:['update world pos', 'camera gap', 'follow ten percent', 'draw slice'], t: "World Coordinates and a Follow Camera", l: "Walk through a four-screen world and draw only one slice by subtracting camera position.", c: "Separate world and screen coordinates", d: "The player stores a world position; only Draw subtracts cameraX. Following ten percent of the gap creates a smooth camera." } },
  { slug: "exploration-map", ja: {s:['部屋へ移動', '訪問済み集合へ保存', '地図へ反映', '未探索を探す'], t: "歩いた部屋だけ広がる地図", l: "部屋番号を訪問済み集合へ記録し、未探索と探索済みを色分けします。", c: "探索状態を部屋IDで保存する", d: "現在位置を部屋幅で割ってroom IDを求めます。一度訪れたIDをboolへ残せば、戻っても地図は消えません。" }, en: {s:['move rooms', 'save visited set', 'reveal map', 'hunt unknown'], t: "A Map that Grows as You Explore", l: "Store visited room IDs and color unexplored and discovered rooms differently.", c: "Save exploration by room ID", d: "Divide world position by room width for the room ID. Keeping visited IDs as booleans preserves the map when the player returns." } },
  { slug: "dash-gate", ja: {s:['dash=falseでは壁', 'フラグ確認', 'dash=trueでX加速', '封印を突破'], t: "能力で開くダッシュゲート", l: "最初は通れない封印を置き、ダッシュ取得後に同じ場所を突破できるようにします。", c: "能力フラグが地形の意味を変える", d: "dashがfalseなら壁として止め、trueならX入力で大きな速度を与えます。同じ地形が取得前は障害、取得後は通路になります。" }, en: {s:['dash false wall', 'check flag', 'dash true burst', 'break the seal'], t: "A Dash-gated Passage", l: "Block a seal at first, then let the player burst through after finding dash.", c: "Ability flags change what terrain means", d: "Stop at the wall while dash is false; with dash true, X grants a large velocity. The same terrain changes from obstacle to route." } },
  { slug: "ability-route", ja: {s:['能力を取得', '過去の障害へ戻る', 'フラグで通過判定', '新しい道が開く'], t: "能力で昔の道を読み替える", l: "地図、ダッシュ、高跳びを順に取得し、以前見た障害へ戻って奥へ進みます。", c: "取得順と戻り道を設計する", d: "能力と対応するゲートを交互に配置します。新能力を得た直後に古い場所を思い出させると、世界が広がったと感じられます。" }, en: {s:['gain ability', 'return to old block', 'flag gate check', 'open new route'], t: "Reinterpret Old Routes with Abilities", l: "Collect the double jump, then return to an earlier high ledge to reach the relic.", c: "Design acquisition order and backtracking", d: "A low route gives the ability first. Returning to the old ledge makes the same space newly playable." } },
];
...
function labMarkup(slug, lang, q) {
  ...
  return `<section class="motion-lab" data-lab="turn" data-states="${q.s.join(",")}"><div><p class="eyebrow">SYSTEM LAB</p><h2>${q.c}</h2><p>${labFlow(lang,q)}</p></div><div class="lab-stage"><div class="lab-entities" data-count="4"></div><p class="lab-readout">START</p><button type="button" class="lab-action">NEXT</button></div></section>`;
}
