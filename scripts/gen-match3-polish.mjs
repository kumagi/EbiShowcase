...
const lessons = [
  { slug: "swap-motion", ja: { s: ["移動開始","接触判定","揃い確認","消去か戻り"], t: "交換をアニメーションにする", l: "2個のピースを動かし、揃う交換は消去へ、揃わない交換は元の場所へ戻します。", c: "交換→判定→消去→落下を分ける", d: "盤面の数字を先に変えず、移動中の割合を0から1へ進めます。接触した瞬間に揃いを調べ、結果に応じて消去または戻りを始めます。" }, en: { s: ["start travel","contact check","match test","clear or return"], t: "Animate Every Swap", l: "Move two pieces, clear a matching swap, and return an invalid swap to its original cells.", c: "Split swap, check, clear, and fall", d: "Do not change the board instantly. Advance a travel ratio from zero to one, check at contact, then clear or return based on the result." } },
  { slug: "reef-director", ja: {s:['ステージ表を読む', '手数を減らす', '揃い消去', '目標点判定', '次の海へ'], t: "3つの海底ステージをつなぐ", l: "手数・目標点・初期盤面・背景をデータにして、難しさの違う3面とベスト記録を作ります。", c: "同じルールへ違う目標データを渡す", d: "ステージ表から名前、手数、目標点、盤面を読みます。クリア時は残り手数をボーナスにし、次の海へ進めることで、一手を大切にしてもう一度遊ぶ理由が生まれます。" }, en: {s:['read stage table', 'spend move', 'clear matches', 'goal check', 'next reef'], t: "Direct Three Reef Stages", l: "Store moves, score goals, opening boards, and backgrounds as data for three escalating stages and a best run.", c: "Feed different goals into the same rules", d: "Read the name, moves, target, and board from a stage table. Turn spare moves into a clear bonus and advance to the next reef, giving every move value and every run a replay target." } },
];
