...
const lessons = [
  {
    ...
    ja: { title: "入力から得点まで、Updateを5つにほどく", lead: "最初のUpdateは、ゲーム開始・時間切れ・タイマー・当たり判定を順番に判断します。短い処理でも、上から読むだけで一つの物語になります。", blocks: [
      ...
      ["遊んでいる時だけ時計を進める", "ここまでreturnされなかった時だけ、残り時間を1 tick減らします。g.ticksLeft-- は g.ticksLeft = g.ticksLeft - 1 の短い書き方です。", "コードの置き場所がルールになります。開始前にはこの行へ届きません。"],
    ...
    en: { title: "Unpack Update from input to score", lead: "This first Update makes four decisions in order: start, timeout, timer, and hit. Even a small function can read like a story from top to bottom.", blocks: [
      ...
      ["Advance the clock only during play", "Only a tick that passed both early returns reaches ticksLeft--. It is shorthand for subtracting one and storing the result.", "Placement is part of the rule: the timer cannot move on the start screen."],
