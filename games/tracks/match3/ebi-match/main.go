...
func (g *game) choose(p point) {
	...
	g.message = fmt.Sprintf("%d pieces! Chain x%d", cleared, g.combo)
...
func (g *game) fallAndRefill() {
	for x := 0; x < cols; x++ {
		...
		for write >= 0 {
			g.board[write][x] = g.rng.Intn(len(pieceColors))
			write--
		}
	}
}
...
func (g *game) Draw(screen *ebiten.Image) {
	...
	for y := 0; y < rows; y++ {
		for x := 0; x < cols; x++ {
			...
			vector.DrawFilledRect(screen, px+3, py+3, cell-6, cell-6, pieceColors[g.board[y][x]], false)
