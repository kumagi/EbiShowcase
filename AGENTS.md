...
- The bilingual lesson-search page (`web/{ja,en}/search/`) and its data file `web/assets/search-index.json` are owned by `scripts/gen-search-index.mjs`. Edit that script (or the source pages it scans) and rerun it; never hand-edit the JSON. Search behavior lives in `web/search.js`.
- Learning-progress badges, the home "resume" card, and the visited-lesson store are implemented in `web/page-boot.js` (covered by `scripts/test-progress-boot.mjs`). Lesson pages record progress when the game frame loads; hub cards read the same localStorage key.
