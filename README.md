...
ターミナルを開くところやGoのインストールから確認したい場合は、先に
[はじめての環境づくり](https://kumagi.github.io/EbiShowcase/ja/guides/setup/)を開いてください。
...
PowerShellでは `pwd` の後に `Get-ChildItem` を実行します。目的のフォルダと違っていたら
`cd フォルダ名` で移動します。Windowsでは `main.go.txt` になっていないか、エクスプローラーの
「表示 → 表示 → ファイル名拡張子」をオンにして確認してください。

### コードを変えたら赤いエラーが出た

壊れたのではなく、Goが直す場所を知らせています。エラーが複数あっても、まず最初の
`main.go:行:列` だけを見ます。その行と一つ前の行で、括弧、引用符、名前のつづりを確認します。
分からなければ最後に変えた箇所だけを元へ戻し、保存してもう一度実行します。

- `undefined: name` — `name` のつづり、または名前を変えた片方だけが古くないか確認
- `expected ...` — 表示された行の直前に `)`、`}`、`"` の閉じ忘れがないか確認
- `imported and not used` — 使わなくなった `import` の1行を削除
