...
SNS 向けの OGP は `node scripts/inject-ogp.mjs` と `go run ./cmd/gen-og-images` で全ページ分を生成します（`build.sh` 内でも実行）。公開 URL のオリジンは環境変数 `SITE_ORIGIN`（省略時 `https://kumagi.github.io/EbiShowcase`）です。
...
`archive` はフォームの回答シートを削除せず、履歴を `feedback_archive_YYYYMMDDHHMM` タブへコピーしてから旧回答行だけを削除します。フォーム連携と見出しは維持されるため、次の投稿は2行目から始まります。
...
- [ ] **卒業プロジェクト brief** 数本（3分ゲーム仕様テンプレ + 近いトラックへの案内）
...
### Phase 5 — 新規ジャンル（必要になってから）

既存トラックで教えにくい「計算の型」があるときだけ追加します。候補の例:
